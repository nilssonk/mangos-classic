...
struct boss_lethonAI : public boss_emerald_dragonAI
{
    ...
    void SpellHitTarget(Unit* pTarget, const SpellEntry* pSpell) override
    {
        ...
        if (pTarget->GetTypeId() == TYPEID_PLAYER && pSpell->Id == SPELL_DRAW_SPIRIT)
        {
            ...
            Creature* pSummoned = pTarget->SummonCreature(
                    NPC_SPIRIT_SHADE,
                    {0.0f, 0.0f, 0.0f, pTarget->GetOrientation()},
                    TempSpawnType::TIMED_OR_DEAD_DESPAWN,
                    60 * IN_MILLISECONDS,
                    SummonFlags{false, false, false, true});
            if (pSummoned != nullptr) {
    ...
    void MoveInLineOfSight(Unit* pWho) override
    {
        ...
        if (pWho->GetEntry() == NPC_LETHON)
        {
            if (!m_bHasHealed && pWho->IsWithinDistInMap(m_creature, 3.0f))
            {
                ...
                m_creature->GetMotionMaster()->MovePoint(0, pWho->GetPosition().xyz());
        }
    }

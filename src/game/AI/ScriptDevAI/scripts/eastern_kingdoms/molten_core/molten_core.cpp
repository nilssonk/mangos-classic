...
void instance_molten_core::SpawnMajordomo(Unit* summoner, bool initialSummon, uint8 summonPos)
{
    ...
    if (Creature* pMajordomo = summoner->SummonCreature(m_aMajordomoLocations[summonPos].id, m_aMajordomoLocations[summonPos].pos, TempSpawnType::MANUAL_DESPAWN, 2 * HOUR * IN_MILLISECONDS))
    {
        ...
        else                                                // Else yell and summon adds
        {
            ...
            for (auto& m_aBosspawnLoc : m_aBosspawnLocs)
                pMajordomo->SummonCreature(m_aBosspawnLoc.id, m_aBosspawnLoc.pos, TempSpawnType::MANUAL_DESPAWN, DAY * IN_MILLISECONDS);
        }
    }
}
